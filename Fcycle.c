...
double v_cycle(double* f, double* u, int num_iterations, int grid_size,int numcyc,int Maxc) {
    // Gauss-Seidel pre-smoothing
    double fac1=pow(0.25,Maxc-numcyc);
    //printf("%f ",fac);
    ...
    for (int i = 0; i < grid_size; i++) {
        ...
        if(i>0 && i<grid_size-1){
            r[i] = f[i] - (u[i+1]-2*u[i]+u[i-1])*fac1;
        }
        else if (i==0)
           r[i] = f[i] - (u[i+1]-2*u[i])*fac1;
        else
            r[i] = f[i] - (-2*u[i]+u[i-1])*fac1;
        //printf("%f ",u[i]);
    }
    ...
    v_cycle( r_coarse, e_coarse, num_iterations, grid_size_coarse,numcyc-1,Maxc);
    ...
    double fac2=pow(0.25,Maxc-numcyc);
...
double fmgcycle(double* f, double* u, int num_iterations, int grid_size,int numcyc) {
    // Gauss-Seidel pre-smoothing
    ...
    // Restrict the residual to the coarse grid
    restrict_residual(r, r_coarse, grid_size);
    //restrict_residual(f, f_coarse, grid_size);
    // Initialize u_coarse with zeros
    for (int i = 0; i < grid_size_coarse; i++) {
        e_coarse[i] = 0.0;
    }
    ...
    fmgcycle(r_coarse, e_coarse, num_iterations, grid_size_coarse,numcyc-1);
   ...
   /* // Gauss-Seidel post-smoothing
    ...
    double* r2 = (double*)malloc(grid_size * sizeof(double));
    for (int i = 0; i < grid_size; i++) {
       
        if(i>0 && i<grid_size-1){
            r2[i] = f[i] - (u[i+1]-2*u[i]+u[i-1])*pow(0.25,Numc-numcyc);
        }
        else if (i==0)
           r2[i] = f[i] - (u[i+1]-2*u[i])*pow(0.25,Numc-numcyc);
        else
            r2[i] = f[i] - (-2*u[i]+u[i-1])*pow(0.25,Numc-numcyc);
        //printf("%f ",u[i]);
    }*/
    
    v_cycle(f, u, num_iterations, grid_size,numcyc,numcyc);
...
int main() {
    ...
    while(norm2>TOLERANCE && iter<MAX_ITER){
        norm2=fmgcycle(f, u, num_iterations, grid_size,numcyc);
        iter+=1;
    }
    //fmgcycle(f, u, num_iterations, grid_size,numcyc);
